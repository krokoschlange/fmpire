...
template<typename T> Arc<T>::Arc() noexcept :
	center(0, 0),
	arc_radius(0),
	start_angle(0),
	end_angle(0),
...
template<typename T> Arc<T>::Arc(const T& x, const T& y, const float radius, const float start, const float end, const uint segments) :
	center(x, y),
	arc_radius(radius),
	start_angle(start),
	end_angle(end),
...
template<typename T> Arc<T>::Arc(const Point<T>& pos, const float radius, const float start, const float end, const uint segments) :
	center(pos),
	arc_radius(radius),
	start_angle(start),
	end_angle(end),
...
template<typename T> Arc<T>::Arc(const Arc<T>& arc) noexcept :
	center(arc.center),
...
template<typename T> void Arc<T>::draw(const GraphicsContext& context, T width)
{
	...
	cairo_arc(handle, center.getX(), center.getY(), arc_radius, rad_start, rad_End);
...
template<typename T> bool Arc<T>::operator==(const Arc<T>& arc) const noexcept
{
	return (center == arc.center
			&& d_isEqual(arc_radius, arc.arc_radius)
...
{
	return (center != arc.center
			|| d_isNotEqual(arc_radius, arc.arc_radius)
